...
LRESULT wnd_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg){ 
		...
		}
		{
			...
			do {
				a--;
				*tp++ = round((width - 1) * sin((double) a * M_PI_2 / 300.0)) * 3;				
			} while (a > 0);
			a = height;
			do {
				a--;
			...
			do {
				a--;
				*tp++ = round((width / 4) * cos((double) a * (M_PI * 2) / 250.0)) * 3;
				*tp++ = - round((width / 4) * sin((double) a * (M_PI * 2) / 250.0)) * 3;				
			} while (a > 0);
			a = height * 2;
			track_end = tp;			
			do {
		...
		
		{
			...
			do {
				...
				if (off < width * -3 || off >= width * 3) {
					WCHAR str[1000];
					_snwprintf(str, sizeof str, L"tp = %d", off);
//					MessageBox(NULL, str,
//						             L"Pocket PC Display Off Feature", MB_OK);
					off = 0;
				}
